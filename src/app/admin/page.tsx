...
export default function AdminPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        setSuccess(`Appartement "${title}" ajouté ! Il sera en ligne dans ~2 minutes.`);
        // Reset form
        setTitle(""); setAddress(""); setDistrict(""); setSurface("");
        setRooms(""); setBedrooms(""); setBathrooms(""); setFloor("");
        setDescription(""); setFeatures(""); setPhotos([]); setPreviews([]);
      ...
      <div className="min-h-screen bg-[#0D0D0D] flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setAuthenticated(true); }} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-3 bg-[#B88B58] text-[#0D0D0D] font-medium tracking-wider uppercase text-sm hover:bg-[#D4AF7A] transition-all"
            >
              Accéder
            </button>
          </form>
    ...
    <div className="min-h-screen bg-[#FAFAF8]">
      ...
      <div className="bg-[#0D0D0D] py-6 px-6">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <a href="/" className="text-[#B88B58] text-sm hover:text-[#D4AF7A] transition-colors">
            ← Retour au site
          </a>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto py-10 px-6">
        ...
        <div className="bg-white border border-[#E8E4DF] p-8 mb-6">
          <h2 className="font-serif text-2xl text-[#1A1A1A] mb-6">Informations principales</h2>

          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs text-[#6B6B6B] uppercase tracking-wider mb-2">Titre *</label>
              <input type="text" required value={title} onChange={(e) => setTitle(e.target.value)}
                placeholder="Rue de Bassano" className="w-full px-4 py-3 border border-[#E8E4DF] text-sm focus:border-[#B88B58] focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-[#6B6B6B] uppercase tracking-wider mb-2">Adresse complète *</label>
              <input type="text" required value={address} onChange={(e) => setAddress(e.target.value)}
                placeholder="Rue de Bassano, Paris 16e" className="w-full px-4 py-3 border border-[#E8E4DF] text-sm focus:border-[#B88B58] focus:outline-none" />
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <input type="text" required value={district} onChange={(e) => setDistrict(e.target.value)}
                placeholder="Paris 16e" className="w-full px-4 py-3 border border-[#E8E4DF] text-sm focus:border-[#B88B58] focus:outline-none" />
            </div>
            <div>
              ...
              <input type="text" value={floor} onChange={(e) => setFloor(e.target.value)}
                placeholder="3e étage avec ascenseur" className="w-full px-4 py-3 border border-[#E8E4DF] text-sm focus:border-[#B88B58] focus:outline-none" />
            </div>
            <div>
        ...
        <div className="bg-white border border-[#E8E4DF] p-8 mb-6">
          ...
          <p className="text-xs text-[#6B6B6B] mb-4">Un équipement par ligne</p>
          <textarea rows={8} value={features} onChange={(e) => setFeatures(e.target.value)}
            placeholder={"Wifi\nTélévision 4K\nLave-vaisselle\nLave-linge séchant\nCuisine équipée\nAscenseur\nParquet\nClimatisation"}
            className="w-full px-4 py-3 border border-[#E8E4DF] text-sm focus:border-[#B88B58] focus:outline-none resize-none font-mono" />
        </div>
        ...
        >
          {loading ? "Publication en cours..." : "Publier l'appartement"}
        </button>
